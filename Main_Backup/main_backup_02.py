...
def generate_pdf(inv_id, save_path):
    ...
    ht = Table([[
        Paragraph(f'<font size="20" color="#E63B2E"><b>{s.get("biz_name","My Business")}</b></font>', styles['Normal']),
    ...
    biz_line2 = ' | '.join(filter(None,[biz_addr, biz_city]))
    biz_line3 = ' | '.join(filter(None,[biz_email, biz_website, biz_phone]))
    biz_details = biz_name
    if biz_line2: biz_details += f'<br/>{biz_line2}'
    if biz_line3: biz_details += f'<br/>{biz_line3}'
    if biz_tagline: biz_details += f'<br/><i>{biz_tagline}</i>'

    biz = f'<font size="9"><b>{biz_name}</b></font><br/><font size="8" color="#6b7280">{biz_details.replace(biz_name,"").lstrip("<br/>")}</font>'
    ...
    pi = f'<font size="10" color="#E63B2E"><b>PAYMENT INFO</b></font><br/><font size="9">{pay_instructions}</font><br/><font size="8" color="#6b7280">{biz_email}</font>'
    ...
    td.append(['', Paragraph('<font size="11" color="#ffffff"><b>TOTAL DUE</b></font>',styles['Normal']),
               Paragraph(f'<font size="11" color="#ffffff"><b>{currency}{inv["total"]:.2f}</b></font>', ParagraphStyle('r',alignment=TA_RIGHT))])
    ...
    story.append(Paragraph(f'<font size="8" color="#6b7280">{" | ".join(footer_parts)}</font>', ParagraphStyle('footer',alignment=TA_CENTER)))
    doc.build(story)
    return True
...
class App(ctk.CTk):
    def __init__(self):
        ...
        if is_first_run():
            ...
            messagebox.showinfo("Welcome!",
                "Welcome to your Invoicer!\n\nPlease fill in your business details\nin Settings before creating invoices.")
        else:
    ...
    def _build_settings(self):
        ...
        field_defs = [
            ("biz_name",     "Business Name *",          "e.g. Green & Green"),
            ("biz_address",  "Street Address",           "e.g. 1122 SW Marine Drive Unit 1"),
            ("biz_city",     "City, Province, Postal",   "e.g. Vancouver, BC V6P5Z3"),
            ("biz_phone",    "Phone Number",             "e.g. 604-000-0000"),
    ...
    def refresh_settings(self):
        s = get_settings()
        simple_keys = ["biz_name","biz_address","biz_city","biz_phone","biz_email","biz_website","tagline","invoice_prefix"]
    ...
    def save_settings(self):
        ...
        biz = get_settings().get("biz_name","Invoicer")
        self.title(f"{biz} — Invoicer")
